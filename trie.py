class Trie:
    ...
    def add_sentence(self, sentence, file_index, sentence_index):
        ...
        for i in range(len(sentence) - 1, -1, -1):
            ...
            if char_index != -1 and char_index != 26:
                ...
                self.add_string(sentence[i:i + 15], index)
    ...
    def small_indexes_repair(self, node, substring, index):
        ...
        """
        for i in range(min(len(substring) - 1, 4), -1, -1):
            for j in range(26):
    ...
    def repair_search(self, node, substring, index, was_repair):
        ...
        if len(substring) == 0 and was_repair:
            if len(node) > 27:
                self.__founded_sentences += map(lambda s: (s, max(index * 2 - was_repair, 0)), node[27])
                self.__founded_sentences = remove_dup(self.__founded_sentences)
            if len(self.__founded_sentences) >= 5:
                return
        ...
        if len(substring) > 0:
            ...
            self.repair_search(node, substring[c + 1:], index, else_decrease)
            if len(self.__founded_sentences) >= 5:
                return
            for i in range(26):
                self.repair_search(node[char_index], chr(i + 97) + substring[c + 1:], index, else_decrease)
                if len(self.__founded_sentences) >= 5:
                    return
                self.repair_search(node[char_index], ' ' + substring[c + 1:], index, else_decrease)
                if len(self.__founded_sentences) >= 5:
                    return
    ...
    def req_search(self, node, substring, index, was_repair):
        ...
        if not was_repair and index > 3:
            ...
            for i in range(27):
                self.req_search(node[i], substring[c + 1:], index, replace_decrease)
                if len(self.__founded_sentences) >= 5:
    ...
    def get_all_children(self, node, index, repair):
        ...
        if (len(self.__founded_sentences) >= 5 and repair == 0) or node == 0:
            return
        if len(node) > 27:
            self.__founded_sentences += map(lambda s: (s, max(index * 2 - repair, 0)), node[27])
            self.__founded_sentences = remove_dup(self.__founded_sentences)
            if len(self.__founded_sentences) >= 5:
                return
        for i in range(27):
