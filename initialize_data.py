...
class Initialize:
    ...
    def parse(self):
        """
        parse on each file in the files directory, for each file ,
        add each line to the data structure (file content- using SQL and trie- using Trie)
        :return:
        """
        file_num = 0
        for root, dirs, files in os.walk(self.__file_name):
            for file in files:
                ...
                with open(file_path, 'r', encoding='utf8') as f:
                    line_num = 0
                    for line in f:
                        # add each line to the files content database and to the trie data structure
    ...
    def create_files_data_table(self):
        """
        create SQL data table for the files content
        """
        try:
    ...
    def insert_file_name(self, file_num, file_name):
        """
        insert the file serial number with its name to the database
        """
        try:
            with connection.cursor() as cursor:
    ...
    def initialize_data(self):
        """
        initialize the data structure, may run only once before beginning the program
        """
        self.create_files_data_table()
        self.parse()
        # write the Trie object to a file
